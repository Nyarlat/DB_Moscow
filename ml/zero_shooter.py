...
class ZeroShooter:
    def __init__(self, model_name: str, device: torch.device = None, knn=None):
        if not device:
    ...
    def recursive_classify_knn(self, text, hierarchy, threshold: float = None, first_level=True):
        if not hierarchy:
            return {}
        ...
        if first_level:
            selected_cats = self.knn.predict(text)
            first_level = False
        else:
            predict = self.classifier(text, current_level, multi_label=True, batch_size=64)
            proba = predict["scores"]
        ...
        for label in selected_cats:
            ...
            if isinstance(next_level, list):
                ...
                result[label] = self.recursive_classify_knn(text, next_level, threshold, first_level=first_level)
